(function(){
  ...
  document.getElementById('order-import-product-list').addEventListener('click',function(event){var button=event.target.closest('[data-link-sku]');if(!button||!importLinkTarget)return;var order=importOrders.find(function(entry){return (entry.orderNo||entry.id||entry.localOrderNo)===importLinkTarget.orderNo});var product=importSkuData.find(function(entry){return entry.barcode===button.dataset.linkSku});if(order&&product){var target=order.items[importLinkTarget.index],wasLinked=!!target.matchedSku,platformSku=target.platformSku||target.sku||'-';if(!target.platformSku)target.platformSku=target.sku||'';target.matchedSku=product.barcode;target.sku=product.barcode;target.productId=product.id||'';target.name=product.name;target.spec=product.spec;target.img=product.img;target.unitPrice=product.unitPrice;target.subtotal=Number((product.unitPrice*target.quantity).toFixed(2));if(order.id&&window.recordOrderOperation){window.recordOrderOperation(order,'association','关联操作',(wasLinked?'重新关联':'关联产品')+'：'+platformSku+' → '+product.barcode);if(order.status==='待关联'&&order.items.every(function(item){return !!item.matchedSku})){order.status='待支付';window.recordOrderOperation(order,'status','订单状态记录','全部商品已关联，订单状态更新为“待支付”',null,'系统')}if(window.persistOrder)window.persistOrder(order);if(window.updateOrderStats)window.updateOrderStats();if(window.applyOrderFilters)window.applyOrderFilters()}else renderImportPreview();closeLink();showToast(wasLinked?'产品重新关联成功':'SKU 关联成功')}});
